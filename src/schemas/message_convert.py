...
from schemas.types import LLMMessage, ClientMessage


def ui_to_llm(msg: ClientMessage) -> LLMMessage:
...
def llm_to_ui(msg: LLMMessage) -> ClientMessage:
    """Convert a LLMMessage to UIMessage for sending to the UI layer."""
    role = "user" if msg.role == "user" else "assistant"
    return ClientMessage(role=role, content=msg.content)
