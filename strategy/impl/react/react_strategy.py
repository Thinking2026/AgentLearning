...
Final Answer: value1 均值 42.50，value2 均值 18.30，已写入 result.txt。"""
    ...
    def get_trimmed_conversation(self, conversation: list[ChatMessage]) -> list[ChatMessage]:
        #1. 根据token预算触发裁剪
        #2. 分槽位，每个槽位分配token预算，系统提示15%，多伦推理35%，工具调用和RAG 35%，用户输入15%
        #3. 引用
        #4. 重要性评分
        #5. 时间衰减
        #6. 滑动窗口+summary
        ...
        #9. summary也要预留token
        #10. 渐进式裁剪，不要上来就压缩
        #11. 保障LLM API需要的tool call/tool result配对

        #token裁剪触发时机：1. build-time hard truncate（必备）2. append-time soft truncate（70% 阈值）3. tool-output 强制压缩（必须）4. step-level summary（ReAct 必备）
        return None
